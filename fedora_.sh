...
build_i3lock-color () {
sudo dnf install autoconf automake cairo-devel fontconfig gcc libev-devel libjpeg-turbo-devel libXinerama libxkbcommon-devel libxkbcommon-x11-devel libXrandr pam-devel pkgconf xcb-util-image-devel xcb-util-xrm-devel -y
cd ${LOCALPATH}
...
build_i3lock-color
sudo dnf install ImageMagick bc xdpyinfo xrandr xrdb xset dunst feh -y
