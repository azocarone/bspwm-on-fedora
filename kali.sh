...
echo -e "${White} ┖───────────────────────────────────────────────┙			"
echo ""
echo -e "\t${White} BSPWM | Hacker environment automation script."
echo -e "\t\t${White} Enríquez González (Aka. AlvinPix)"
echo ""
echo -e "${White} Installation will begin soon."
echo ""
sleep 4
...
if [ $quest = Y ]; then
	...
	if which polybar >/dev/null; then
		...
		echo ""
		echo -e "${White} [${Blue}i${White}] Step 13 Installing the powerlevel10k, fzf, sudo-plugin, and others for the root user"
		sleep 2
		echo ""
		sudo cd ${RUTE}/root
		sudo cp -r .zshrc .p10k.zsh /root
		sudo cd /root ; git clone --depth=1 https://github.com/romkatv/powerlevel10k.git ~/powerlevel10k
		echo ""
	        cd ${RUTE}
                sudo cp -r scripts /root
		sudo cd /root/scripts ; git clone https://github.com/charitarthchugh/shell-color-scripts.git
		echo ""
		sudo rm -rf /root/scripts/shell-color-scripts/colorscripts
                sudo rm -rf /root/scripts/shell-color-scripts/colorscript.sh
                sudo cd /root/scripts
		sudo mv colorscripts colorscript.sh /root/scripts/shell-color-scripts
		sudo chmod +x /root/scripts/shell-color-scripts/colorscript.sh
		sudo cd /root/scripts/shell-color-scripts/colorscripts
		sudo chmod +x *
		sudo cd /root
		echo ""
		sudo git clone --depth 1 https://github.com/junegunn/fzf.git ~/.fzf
                ~/.fzf/install
		echo ""
		sudo cd /root/scripts ; git clone https://github.com/pipeseroni/pipes.sh.git
		echo ""
fi
}
